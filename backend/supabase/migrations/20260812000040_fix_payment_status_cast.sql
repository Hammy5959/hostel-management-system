-- ─────────────────────────────────────────────────────────────────────────────
-- HMS — Fix hms_record_payment: enum cast for invoice status.
--
-- The original function set invoice status via a CASE expression that produced
-- `text`, but the column is the `invoice_status` enum, so the UPDATE failed
-- with: "column \"status\" is of type invoice_status but expression is of type
-- text". This CREATE OR REPLACE restores the corrected function.
